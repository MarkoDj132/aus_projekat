...
namespace Modbus.Connection
{
    ...
    public class FunctionExecutor : IDisposable, IFunctionExecutor
	{
        ...
        public void EnqueueCommand(IModbusFunction commandToExecute)
		{
            ...
            //stateUpdater.LogMessage("ENQUEUE CALLED");

            if (this.connectionState == ConnectionState.CONNECTED)
            {
                //stateUpdater.LogMessage("ENQUEUE ACCEPTED");
            ...
            {
                //stateUpdater.LogMessage("ENQUEUE REJECTED - NOT CONNECTED");
            }
        }
		...
		{
            stateUpdater.LogMessage("RAW: " + BitConverter.ToString(receivedBytes));

            //stateUpdater.LogMessage("RESPONSE RECEIVED: " + currentCommand.GetType().Name);
		...
		private void ConnectionProcessorThread()
		{
			while (this.threadCancellationSignal)
			{
				try
				{
					...
					{
						//---
                        //stateUpdater.LogMessage("TCP THREAD ACTIVE");
						//---
                        processConnection.WaitOne();
						while (commandQueue.TryDequeue(out currentCommand))
						{
                            //stateUpdater.LogMessage("COMMAND DEQUEUED");

                            //stateUpdater.LogMessage("SENDING BYTES");
                            this.connection.SendBytes(this.currentCommand.PackRequest());
                            //stateUpdater.LogMessage("BYTES SENT");
